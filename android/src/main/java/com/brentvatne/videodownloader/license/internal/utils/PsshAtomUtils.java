 ...
 */
package com.brentvatne.videodownloader.license.internal.utils;

import android.util.Log;

import com.brentvatne.videodownloader.license.internal.model.Atom;
 ...
 */
final class PsshAtomUtils {

    private static final String TAG = "PsshAtomUtils";

    private PsshAtomUtils() {
    }
     ...
     */
    static byte[] buildPsshAtom(UUID[] keyIds, @SuppressWarnings("SameParameterValue") byte[] data) {
        ...
        psshBox.putLong(androidx.media3.common.C.COMMON_PSSH_UUID.getMostSignificantBits());
        psshBox.putLong(androidx.media3.common.C.COMMON_PSSH_UUID.getLeastSignificantBits());
        if (buildV1Atom) {
     ...
     */
    static UUID parseUuid(byte[] atom) {
     ...
     * an unsupported version.
     */
    static int parseVersion(byte[] atom) {
        PsshAtom parsedAtom = parsePsshAtom(atom);
        if (parsedAtom == null) {
            return -1;
        }
     ...
     */
    static byte[] parseSchemeSpecificData(byte[] atom, UUID uuid) {
        PsshAtom parsedAtom = parsePsshAtom(atom);
        if (parsedAtom == null) {
            return null;
        }
